...
-- 데이터 타입(도메인 제약조건)에 맞는 데이터를 입력하면 정상 수행ALTER
...
-- 기본키에 중복되는 값을 입력할 경우 제약조건 위반(개체 무결성)
...
-- 복합 기본키의 각 컬럼은 중복되어도, 두 기본키의 조합이 중복 되지않으면 제약조건 위반하지 않음
...
CREATE TABLE users(
    ...
    usetname VARCHAR(50) NOT NULL,	-- 필수(Not null)
